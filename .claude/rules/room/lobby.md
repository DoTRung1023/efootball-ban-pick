...
`.prep-scroll` is where the settings live and the only thing in the column that
may scroll. Do not move `overflow-y` onto `.prep-col--settings`: that is what let
the "BAN SETTING" header scroll out of view.

**A guest whose column scrolls differently from the host's is two windows either
side of the `max-height: 820px` rung**, not a host/guest difference. The guest's
...
every interactive element inside the scroller, `.lv-stepper-btn` and `.lv-reveal-card`
included — no extra CSS needed for new controls. `renderLobby()` additionally sets
`.disabled` on the two duration inputs directly (`banDurationEl.disabled = !isHost`).
...
## Removed: category allowance

The settings panel used to carry a second section — CATEGORY ALLOWANCE — where
the host added filter categories (position, club, foot, age, …), gave each one a
min and a max number of players, and the draft enforced them: a maximum as a card
was placed, a minimum at CONFIRM. It was thirteen categories over four control
shapes, a `{value: count}` map per end, a duplicated normaliser on the server and
a filtered pick pool, and it was removed as more machinery than the draft wanted.

A room's config is now five fields: `banCountPerSide`, `banDurationSec`,
`pickDurationSec`, `revealMode`, `pickCountPerSide`. You ban from your opponent's
roster and pick from your own; nothing else constrains a squad. Do not reintroduce
a rule engine here without being asked for one — `git log` has the whole thing if
